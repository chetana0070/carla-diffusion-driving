#!/usr/bin/env python3
"""Train and evaluate the Phase 4 single-frame behavioral-cloning baseline."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "artifacts" / "checkpoints" / "phase4_single_frame_bc",
    )
...
def make_loader(
    dataset: SingleFrameWindowDataset,
    *,
    batch_size: int,
    workers: int,
    shuffle: bool,
...
def main() -> int:
    ...
    bc_config = config["behavioral_cloning"]
    seed = int(config["project"]["random_seed"])
    seed_everything(seed)

    if not torch.cuda.is_available() and not args.allow_cpu:
        raise RuntimeError("CUDA is required; pass --allow-cpu only for diagnostic smoke tests")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    epochs = int(args.epochs or bc_config["epochs"])
    batch_size = int(args.batch_size or bc_config["batch_size"])
    learning_rate = float(args.learning_rate or bc_config["learning_rate"])
    ...
    model = SingleFrameBC(
        scalar_dimension=int(bc_config["scalar_input_dimension"]),
        scalar_feature_dimension=int(bc_config["scalar_feature_dimension"]),
        pretrained=args.pretrained and not args.smoke,
    ).to(device)
    freeze_epochs = 0 if args.smoke else int(bc_config["freeze_encoder_epochs"])
    ...
    for epoch in range(1, epochs + 1):
        ...
        validation = evaluate(
            ...
            loaders["validation"],
            device,
            steering_tolerance=float(bc_config["steering_tolerance"]),
            longitudinal_tolerance=float(bc_config["longitudinal_tolerance"]),
