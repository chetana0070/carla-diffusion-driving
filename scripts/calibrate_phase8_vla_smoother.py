...
def main() -> int:
    ...
    smoothing = project_config["vla_steering_smoothing"]
    ...
    derived_checkpoint["phase8_3_config"] = dict(smoothing)
    ...
    report = {
        "status": "passed",
        "model_type": "hierarchical_vla_smoothed",
        "mode": "validation_only_calibration",
        ...
        "elapsed_wall_seconds": round(time.perf_counter() - started, 3),
        "claim_boundary": (
            "Validation-calibrated deterministic steering postprocessor; test data was "
            "not read and no closed-loop claim is made."
        ),
    }
