#!/usr/bin/env python3
"""Measure batch-one deterministic DDIM latency for a Phase 7 checkpoint."""
...
def main() -> int:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for the diffusion latency benchmark")
    device = torch.device("cuda")
    checkpoint: dict[str, Any] = torch.load(
        args.checkpoint, map_location=device, weights_only=False
    )
    if checkpoint.get("model_type") != "temporal_diffusion_policy":
        raise ValueError("checkpoint is not a temporal diffusion policy")
    project_config = checkpoint["config"]
    config = project_config["diffusion_policy"]
    model: TemporalDiffusionPolicy = build_model(config, pretrained=False).to(device)
    ...
    if not bool(torch.all(torch.isfinite(prediction))):
        raise FloatingPointError("diffusion benchmark produced non-finite actions")
    summary = latency_summary(latencies)
    budget = float(project_config["evaluation"]["max_policy_latency_ms"])
    report = {
        "status": "passed",
        "scope": "observation_encoding_plus_ddim_sampling",
